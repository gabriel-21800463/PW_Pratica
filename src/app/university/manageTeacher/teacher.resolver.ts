...
export class TeacherResolver implements Resolve<ITeacher> {
  constructor(private router: Router, private spinner: NgxSpinnerService, private teacher: TeacherService) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ITeacher> | Observable<never> {
    const id = route.params.id;
    if (id) {
      this.spinner.show();
      return this.teacher.getTeacherById(id).pipe(map((data: ITeacher) => {
        this.spinner.hide();
        if (data) {
          return data;
        } else {
